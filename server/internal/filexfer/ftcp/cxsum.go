...
func handleCXSUM(_ context.Context, req Request, out io.Writer, deps Deps) error {
	...
	metadata := intframe.CollectFileFrameMetadata(fileRef.Path, fileInfo)
	full128 := xxh3.New128()
	full64 := xxh3.New()
	...
	if fileSize == 0 {
		...
		_, err := intframe.WriteFrame(out, intframe.WriteArgs{
	...
	for cursor < fileSize {
		...
		var meta *intframe.FileFrameMetadata
		if isTerminal {
			nextValue = 0
			meta = &metadata
		}
...
func finalChecksumTokens(algorithms []string, full128 *xxh3.Hasher128, full64 *xxh3.Hasher) []string {
	...
	for _, name := range algorithms {
		switch name {
		case "xxh128":
			tokens = append(tokens, intcodec.FormatXXH128HashToken(full128.Sum128()))
		case "xxh64":
			tokens = append(tokens, intcodec.FormatXXH64HashToken(full64.Sum64()))
		}
	}
	return tokens
}
