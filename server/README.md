...
FD=$(curl -s 'localhost:8080/pinch?min-level=1&timeout=200' | jq '.handles | keys[0]' -r)
RFD=$(curl -s 'localhost:8080/unpinch?timeout=200' | jq '.handles | keys[0]' -r)
...
```bash
$ curl -s 'localhost:8080/pinch?timeout=200&age-public-key=age1630vztsaydze8r9qc3e865spc989mvcls3wg7hh9vu2w3luulqlqp0v6wh' | jq .
{
  "handles": {
    "14ccbb285f4d5822": {
      "io-http": "http://127.0.0.1:8080/write/14ccbb285f4d5822",
...
$ curl -sT yelp_academic_dataset_business.json 'localhost:8080/write/14ccbb285f4d5822?c&rw' > business.zst.age
...
$ curl -s 'localhost:8080/unpinch?age-key-path=private&timeout=200' | jq .
{
  "handles": {
    "0d2eeb7e0819c098": {
      "io-http": "http://127.0.0.1:8080/write/0d2eeb7e0819c098",
...
$ curl -s 'localhost:8080/unpinch?age-key-path=private&timeout=200' | jq .
{
  "handles": {
    "1805b02fce8ecca1": {
      "io-http": "http://127.0.0.1:8080/write/1805b02fce8ecca1",
...
$ curl -sT business.zst.age 'localhost:8080/write/1805b02fce8ecca1?c&rw' | head
