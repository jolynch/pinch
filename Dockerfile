...
RUN git clone --depth=1 --branch v1.10.0 https://github.com/lz4/lz4.git         /lz4_src
RUN git clone --depth=1 --branch v1.5.7  https://github.com/facebook/zstd.git   /zstd_src
RUN git clone --depth=1 --branch v0.8.3  https://github.com/Cyan4973/xxHash.git /xxh_src
...
RUN wget https://github.com/BLAKE3-team/BLAKE3/releases/download/1.8.3/b3sum_linux_x64_bin -O /usr/local/bin/b3sum
RUN chmod +x /usr/local/bin/b3sum
RUN strip /usr/local/bin/b3sum

RUN wget https://github.com/FiloSottile/age/releases/download/v1.3.1/age-v1.3.1-linux-amd64.tar.gz
RUN tar -xvvzf age-v1.3.1-linux-amd64.tar.gz
